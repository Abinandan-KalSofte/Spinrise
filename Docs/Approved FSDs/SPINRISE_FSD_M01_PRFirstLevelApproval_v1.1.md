...
+:-----------------+:-----------------+:----------------------------------------+:-----------------+:-------------------------------------+
| **Critical Defect Audit Evidence (Stage 0 FULLY APPROVED by TL-Dev Sasi 21 May 2026 · CD-01 through CD-06 confirmed)**                  |
+------------------+------------------+-----------------------------------------+------------------+--------------------------------------+
| **Finding**      | **Issue**        | **Detail**                              | **Risk**         | **Resolution / CD Raised**           |
+------------------+------------------+-----------------------------------------+------------------+--------------------------------------+
| **CD-01**        | **G4 --- SQL     | All SQL in BUTTON_Click, DepLookup,     | **HIGH**         | NOTED ✔ Stage 0 confirmed by Sasi:   |
...
|                  |                  |                                         |                  | in SPINRISE.                         |
+------------------+------------------+-----------------------------------------+------------------+--------------------------------------+
| **CD-03**        | **G5 --- Eight   | MSDATGRD.OCX (grddatagrid),             | **HIGH**         | NOTED ✔ Tracked as G5 per Blueprint  |
...
+------------------+------------------+-----------------------------------------+------------------+--------------------------------------+
| **CD-02**        | **G7 ---         | stock_find performs a DB lookup on      | **MED**          | NOTED ✔ Stage 0 confirmed by Sasi:   |
...
|                                     | IDE.                                           |
+-------------------------------------+------------------------------------------------+
| **FSD Version**                     | v1.1: Initial FSD --- frmindentapp.frm (PR     |
...
|                                     | concatenation. SPINRISE introduces two SPs for |
|                                     | First Level Approval only:                     |
|                                     | sp_PR_GetPendingFirstApproval (@divcode        |
|                                     | varchar(10), \@dep varchar --- no year guard), |
|                                     | sp_PR_GetFirstApprovedForDeletion (@divcode    |
|                                     | varchar(10) --- no year guard). SPs for Second |
|                                     | and Third Level Approval belong to separate    |
|                                     | FSDs.                                          |
+-------------------------------------+------------------------------------------------+
...
|                                     | Concurrency: rowversion on PO_PRH / PO_PRL     |
|                                     | recommended (see OI-03) │ Division-scoped: all |
|                                     | queries filtered by divcode │ User-level       |
|                                     | enforced at API level (see CD-05)              |
+-------------------------------------+------------------------------------------------+
...
+---------------+------------------------------------------+------------------------------------------+----------------+---------------+---------------+
| **Listing     | BUTTON_Click Add (Case 0): checkrs query | Dapper SP sp_PR_GetPendingFirstApproval  | Add / Modify   | READ PO_PRH,  | **CHANGED**   |
| query ---     | opens 3 variants by approval level: (1)  | (@divcode, \@dep) returns PRs eligible   | --- listing    | PO_PRL,       |               |
| level-aware** | All                                      | for First Level approval (all approval   | step           | in_dep,       |               |
|               | FirstApp/SecondApp/ThirdApp/DirectApp IS | flags NULL, cancelflag\<\>\'Y\'). React  |                | in_item       |               |
|               | NULL = unapproved; (2) FirstApp NOT      | modal table shows results. If 0 records: |                |               |               |
|               | NULL, rest NULL = pending SecondApp; (3) | toast \'No Records Found\'.              |                |               |               |
...
|                                     | Approval Level.                                |
+-------------------------------------+------------------------------------------------+
| **Re-print Control**                | Re-print via PO_PRINT_LOG / REPRINT_FLG ---    |
|                                     | confirm applicability with Sasi at Stage 2.    |
|                                     | See OI-04.                                     |
+-------------------------------------+------------------------------------------------+
...
+:------------------------------------+:-----------------------------------------------+
| **6 Customer Variations (ig_param Configuration --- Zero CustID Blocks in            |
| SPINRISE)**                                                                          |
+-------------------------------------+------------------------------------------------+
| **ig_param Flags**                  | (None) --- frmindentapp.frm has no ig_param    |
|                                     | flags and no CustID blocks --- confirmed by    |
|                                     | grep scan. G1 PASS, G9 PASS (preliminary).     |
+-------------------------------------+------------------------------------------------+
...
|                                       |                    |                    | --- no \@yfdate / \@yldate     |
|                                       |                    |                    | parameters. Listing query: PRs |
|                                       |                    |                    | where FirstApp IS NULL AND     |
|                                       |                    |                    | SecondApp IS NULL AND ThirdApp |
|                                       |                    |                    | IS NULL AND DirectApp IS NULL  |
|                                       |                    |                    | AND cancelflag\<\>\'Y\' AND    |
|                                       |                    |                    | FClosed\<\>\'Y\',              |
|                                       |                    |                    | divcode=@divcode. Department   |
|                                       |                    |                    | filter \@dep applied per user  |
|                                       |                    |                    | level. SP name confirmed by    |
|                                       |                    |                    | Sasi (OI-02 CLOSED).           |
+---------------------------------------+--------------------+--------------------+--------------------------------+
...
|                                       |                    |                    | \@yldate parameters. Listing   |
|                                       |                    |                    | query: PRs where FirstApp IS   |
|                                       |                    |                    | NOT NULL AND DirectApp IS NULL |
|                                       |                    |                    | AND cancelflag\<\>\'Y\'.       |
|                                       |                    |                    | Supports Delete-First-Approval |
|                                       |                    |                    | listing. SP name confirmed by  |
|                                       |                    |                    | Sasi (OI-02 CLOSED).           |
+---------------------------------------+--------------------+--------------------+--------------------------------+
...
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
| **OI-02**     | **SPINRISE SP     | Only TWO stored procedures apply  | **CLOSED**    | Sasi (Stage   | CREATE new SPs   |
|               | names for         | to PR First Level Approval.       |               | 2)            | per Section 6.   |
|               | approval listing  | Confirmed SPs: (1)                |               |               | No existing VB6  |
|               | queries**         | sp_PR_GetPendingFirstApproval --- |               |               | SPs.             |
...
|               |                   | sp_PR_GetPendingSecondApproval    |               |               |                  |
|               |                   | and sp_PR_GetPendingThirdApproval |               |               |                  |
|               |                   | are removed from this FSD ---     |               |               |                  |
|               |                   | they belong to Second and Third   |               |               |                  |
|               |                   | Level Approval forms only.        |               |               |                  |
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
| **OI-03**     | **Concurrency --- | The approval transaction reads    | **OPEN**      | Sasi / CEO    | If required:     |
|               | rowversion on     | then writes PO_PRL lines. If two  |               |               | ALTER TABLE      |
|               | PO_PRH / PO_PRL** | approvers work on the same PR     |               |               | PO_PRH ADD       |
|               |                   | simultaneously, last-write-wins   |               |               | row_version      |
|               |                   | without a rowversion column.      |               |               | ROWVERSION;      |
|               |                   | Confirm whether ALTER TABLE to    |               |               | ALTER TABLE      |
|               |                   | add rowversion is required before |               |               | PO_PRL ADD       |
|               |                   | go-live.                          |               |               | row_version      |
|               |                   |                                   |               |               | ROWVERSION. No   |
|               |                   |                                   |               |               | data migration   |
|               |                   |                                   |               |               | needed.          |
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
| **OI-04**     | **Re-print        | RESOLVED per Blueprint §12        | OPEN ---      | Sasi (Stage   | Verify           |
|               | control ---       | Architecture Standard: Re-print   | schema        | 2)            | PO_PRINT_LOG     |
|               | RepPO.rpt**       | control via PO_PRINT_LOG /        | confirm only  |               | schema columns.  |
|               |                   | REPRINT_FLG is a mandatory        |               |               | No ALTER on      |
|               |                   | architecture standard for all     |               |               | PO_PRH/PO_PRL.   |
|               |                   | forms (Blueprint §12). SPINRISE   |               |               | If PO_PRINT_LOG  |
|               |                   | QuestPDF class PrApprovalReport   |               |               | table does not   |
|               |                   | must: (1) INSERT into             |               |               | exist: CREATE    |
|               |                   | PO_PRINT_LOG on every print event |               |               | TABLE per        |
|               |                   | (prno, prdate, divcode, userid,   |               |               | architecture     |
|               |                   | print_date, print_count). (2)     |               |               | standard before  |
|               |                   | Check REPRINT_FLG before allowing |               |               | go-live.         |
|               |                   | reprint. (3) First print always   |               |               |                  |
|               |                   | allowed. Reprint requires         |               |               |                  |
|               |                   | REPRINT_FLG permission for the    |               |               |                  |
|               |                   | user. Sasi to confirm             |               |               |                  |
|               |                   | PO_PRINT_LOG schema at Stage 2.   |               |               |                  |
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
...
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
| **OI-07**     | **Delete-Approval | CEO confirmed (24 May 2026): Only | **CLOSED**    | CEO T. Mani   | No schema        |
|               | --- role          | a user with DeleteApproval role   |               |               | change. SPINRISE |
|               | permission and    | permission may execute a          |               |               | API: check       |
|               | self-delete       | delete-approval. The same user    |               |               | DeleteApproval   |
|               | restriction**     | who approved may not self-delete  |               |               | role             |
|               |                   | their own approval. This is a     |               |               | permission +     |
|               |                   | standard audit control. SPINRISE  |               |               | anti-self-delete |
|               |                   | API must enforce: (1) user has    |               |               | guard before     |
|               |                   | DeleteApproval role permission    |               |               | executing        |
|               |                   | before permitting delete-approval |               |               | delete-approval  |
|               |                   | action, (2) Trans_UserId of       |               |               | DML.             |
|               |                   | delete must not match             |               |               |                  |
|               |                   | Trans_UserId of the original      |               |               |                  |
|               |                   | approval. LogDet_PO               |               |               |                  |
|               |                   | Trans_Mod=\'DELETE\' to record    |               |               |                  |
|               |                   | the delete-approval event.        |               |               |                  |
+---------------+-------------------+-----------------------------------+---------------+---------------+------------------+
