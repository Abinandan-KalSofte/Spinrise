...
BEGIN
    ...
    SELECT
        ...
        RTRIM(ISNULL(cby.user_name, ''))                            AS CreatedBy,
    ...
    OUTER APPLY (SELECT TOP 1 user_name FROM dbo.PP_PASSWD
                 WHERE RTRIM(user_id) = RTRIM(h.createdby)
                   AND user_name IS NOT NULL AND LTRIM(RTRIM(user_name)) <> ''
                   -- TEMP HARDCODE: PO Entry module = '3'. Confirm/parameterise later.
                   AND RTRIM(CAST(module AS VARCHAR(10))) = '3'
                 ORDER BY CASE WHEN RTRIM(divcode) = RTRIM(h.DIVCODE) THEN 0 ELSE 1 END,
                          divcode)                                         cby
