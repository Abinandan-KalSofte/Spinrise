...
public sealed class PendingPrDeptWiseDocument : IDocument
{
    ...
    private const float W1  = 38;
    private const float W2  = 50;
    private const float W3  = 60;
    private const float W4  = 180;
    private const float W5  = 35;
    private const float W6  = 70;
    private const float W7  = 70;
    private const float W8  = 70;
    private const float W9  = 50;
    private const float W10 = 190;

    private readonly IReadOnlyList<PendingPrDeptWiseRowDto> _rows;
    private readonly PrReportRequest _request;
    private readonly string _printName;
    private readonly string _unitName;

    public PendingPrDeptWiseDocument(IReadOnlyList<PendingPrDeptWiseRowDto> rows, PrReportRequest request)
    {
        _rows      = rows;
        _request   = request;
    ...
    {
        Title  = "Pending Purchase Requisition Report - Department Wise",
        Author = "SpinRise",
    };
    ...
    private void ComposeTitle(IContainer container)
    {
        container.Column(col =>
        {
            ...
            col.Item().PaddingTop(4).Layers(layers =>
            {
                ...
                {
                    t.AlignCenter();
                    t.Span("Option : Pending PR  —  Department wise").FontSize(8).Bold().FontColor(Teal);
                });
            });
    ...
    private void ComposeContent(IContainer container)
    {
        ...
        var ordered = _rows
            .OrderBy(r => r.DepCode, StringComparer.OrdinalIgnoreCase)
            .ThenBy(r => ParsePrNo(r.PrNo))
            .ThenBy(r => r.PrDate)
            .ThenBy(r => r.ItemName, StringComparer.OrdinalIgnoreCase)
            .ThenBy(r => r.Uom, StringComparer.OrdinalIgnoreCase)
            .ToList();
        ...
        container.Table(table =>
        {
            ...
            table.Header(h =>
            {
                ...
                HCell(h, "Remarks");
                h.Cell().ColumnSpan(10).PaddingTop(1).LineHorizontal(0.75f).LineColor(Black);
            });
            ...
            foreach (var g in ordered.GroupBy(r => (r.DepCode, r.DepName)))
            {
                ...
                table.Cell().ColumnSpan(10).PaddingLeft(2).PaddingTop(4).PaddingBottom(1)
                ...
                foreach (var r in g)
                {
                    ...
                    DCell(table, r.Remarks ?? "");
                }
            }

            table.Cell().ColumnSpan(10).PaddingTop(2).LineHorizontal(0.75f).LineColor(Black);
        });
    }
