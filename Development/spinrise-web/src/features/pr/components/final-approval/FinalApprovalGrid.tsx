...
export default function FinalApprovalGrid() {
  ...
  const allSelected =
    selectableLines.length > 0 && selectableLines.every((l) => l.selected);
    ...
    (idx: number, val: number | null) => {
      ...
      if (val === null || val <= 0) {
        Modal.warning({
          ...
          content:
            "Approved quantity must be greater than zero. Value reset to 1.",
        });
        updateQty(idx, 1);
        return;
      }
  ...
  const handleDispositionChange = useCallback(
    (idx: number, code: DispositionCode) => {
          ...
          >
            <span
              style={{
                ...
                background: ROW_COLORS[s],
                border: "1px solid #ccc",
              }}
            />
            <span style={{ color: "#555" }}>
              {s === "first"
                ? approvalLabels.appUserLabel1
                : s === "second"
                  ? approvalLabels.appUserLabel2
                  : approvalLabels.appUserLabel3}
            </span>
          </span>
        ))}
        {someSelected && (
        ...
        >
          <thead>
            <tr>
              ...
              <th style={TH({ width: 86, textAlign: "right" })}>
                Approx. Value
              </th>
            </tr>
          </thead>
            ...
            {lines.map((line, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Div */}
                  <td style={TD({ width: 40, fontSize: 11, color: "#555" })}>
                    {line.divCode}
                  </td>
                  ...
                  {/* Department */}
                  <td style={TD({ width: 100, fontSize: 11 })}>
                    {line.department}
                  </td>

                  {/* Item Name */}
                  <td style={TD({ minWidth: 70, maxWidth:100 })}>{line.itemName}</td>
                  ...
                  <td style={TD({ width: 112 })}>
                    <Select
                      ...
                      style={{ width: "100%" }}
                      onChange={(v) =>
                        handleDispositionChange(idx, v as DispositionCode)
                      }
                    />
                  </td>
