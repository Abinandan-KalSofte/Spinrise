...
import { triggerDownload } from '../../pr/api/prReportApi'
...
export function ReportPreviewModal({ open, blobUrl, filename, loading, onClose, title = 'Report Preview' }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Viewer area */}
      {/* alignItems must stay 'flex-start' here: 'center' on a scrollable flex
          container clips overflow on one side and makes it unreachable by
          scroll once zoomed content is wider than the container. Centering is
          done per-page instead (margin: 0 auto on the page wrapper below). */}
      <div
        ref={scrollRef}
        style={{
          flex:           1,
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    lineHeight:    0,
                    flexShrink:    0,
                    // Centers the page when narrower than the viewer, while still
                    // letting the viewer scroll to the full width when zoomed in
                    // wider than the viewer (see alignItems note on the container).
                    margin:       '0 auto',
