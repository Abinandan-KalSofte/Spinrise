...
using SpinRise.QuestPDF.DepartmentWise;
using SpinRise.QuestPDF.DepartmentWise.Sample;

namespace SpinRise.QuestPDF.Host.Reports;

/// <summary>
/// Host-side adapter for the Department-Wise report. Turns the parsed CLI args and
/// appsettings.json into <see cref="DepartmentWiseParameters"/> and invokes the
/// underlying report (live or demo).
/// </summary>
internal sealed class DepartmentWiseRunner : IReportRunner
{
    ...
    {
        // Department range follows the SP's @FrmDep/@ToDep. A single --dep maps to both ends;
        // otherwise --fromdep/--todep (or config) set the range; null/null = all departments.
        ...
        if (cli.Demo)
        {
            var rows = DepartmentWiseSampleData.Build();
            return new DepartmentWisePdfReport(null!).Render(rows, parameters);
        }
        ...
        return await new DepartmentWisePdfReport(repository).GenerateAsync(parameters, ct);
    }
